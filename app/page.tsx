...
export default function Home() {
 ...
 function chooseBand(index:number){setPlaying(false);setBandIndex(index);setOriginal(false);}
 ...
 <header className="topbar"><a className="brand" href="/" aria-label="Spectrum home"><span className="brand-mark" aria-hidden="true"><i/><i/><i/><i/></span>Spectrum<span className="brand-label">ELECTROMAGNETIC EXPLORER</span></a><button className="header-note about-button" onClick={()=>setAbout(true)}><Info size={15}/> About the simulations</button></header>
 <main>
 ...
 <SweepControls playing={playing} onPlaying={value=>{setPlaying(value);if(value){setOriginal(false);setView('explore');}}} speed={speed} onSpeed={setSpeed} disabled={!ready}/>
 ...
 <p className="mode-copy">{original?'The world as your camera sees it. Your original photo, with no spectrum transformation.':band.description}</p>
 ...
 <EffectControls options={effects} onChange={value=>{setEffects(value);setOriginal(false);setPlaying(false);}} disabled={!ready} bandId={band.id}/>
